...
public class RulerView extends View {
    ...
    private float mPerValue = 1; //最小单位 如 1:表示 每2条刻度差为1. 0.1:表示 每2条刻度差为0.1 // 在demo中 身高mPerValue为1 体重mPerValue 为0.1
    ...
    private float mTextMarginTop = 0; //o
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < mTotalLine; i++) {
            ...
            if (i % 10 == 0) {
                ...
                Log.d("======text",mTextPaint.measureText(value)+"");
                canvas.drawText(value, left - mTextPaint.measureText(value) / 2, mLineMaxHeight+mTextHeight-mTextMarginTop, mTextPaint); // 在为整数时,画 数值
            }
                 /*线条*/
            if (i % 10 == 0) {
                height = 0;
            } else if (i % 5 == 0) {
                height = mLineMaxHeight-mLineMidHeight;
            } else {
                height =  mLineMaxHeight-mLineMinHeight;
            }
            if (mAlphaEnable) {
            ...
            canvas.drawLine(left, mLineMaxHeight , left, height, mLinePaint);
